...
test('privacy page loads with content', async ({ page }) => {
  await page.goto('/privacy', { waitUntil: 'domcontentloaded', timeout: 20000 });
  await expect(page.locator('main, [role="main"], article, section').first()).toBeVisible({ timeout: 15000 });
});

test('data-deletion page loads', async ({ page }) => {
  await page.goto('/data-deletion', { waitUntil: 'domcontentloaded', timeout: 20000 });
  await expect(page.locator('main, [role="main"]').first()).toBeVisible({ timeout: 15000 });
});
