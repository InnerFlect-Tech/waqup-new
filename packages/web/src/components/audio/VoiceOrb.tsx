...
const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    vec3 innerColor = uColorPrimary * (0.4 + 0.6 * uIntensity);
    vec3 rimColor = uColorSecondary * (0.6 + 0.4 * uIntensity);

    vec3 color = mix(innerColor, rimColor, fresnel);

    float pulse = 0.85 + 0.15 * sin(uTime * 1.5) * (1.0 - uIntensity * 0.5);
    color *= pulse;

    float alpha = 0.7 + 0.3 * fresnel + 0.2 * uIntensity;
