...
/**
 * Determine if color is dark (for text color selection)
 */
...
/**
 * Get optimal text color for a background (ensures WCAG AA contrast)
 */
function getOptimalTextColor(backgroundColor: string, preferLight: boolean = false): string {
  const isBgDark = isDark(backgroundColor);
  
  // If background is dark, use light text; if light, use dark text
  if (isBgDark) {
    // Ensure white text has good contrast (at least 4.5:1)
    const whiteContrast = getContrastRatio('#FFFFFF', backgroundColor);
    if (whiteContrast >= 4.5) {
      return '#FFFFFF';
    }
    // If white doesn't have enough contrast, use a lighter shade
    return '#F5F5F5'; // Very light gray
  } else {
    // Ensure black text has good contrast
    const blackContrast = getContrastRatio('#000000', backgroundColor);
    if (blackContrast >= 4.5) {
      return '#000000';
    }
    // If black doesn't have enough contrast, use a darker shade
...
/**
 * Generate a complete theme from 6 variables
 */
