...
const STEP_LABELS: Partial<Record<ConversationStep, string>> = {
  intent: 'Intent',
  context: 'Context',
...
function OrbPageInner() {
  ...
  const startListening = useCallback(() => {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
      setBrowserError('Speech recognition is not supported in this browser. Please use Chrome or Edge.');
      return;
    }
  ...
  const getOrbPromptForStep = useCallback((type: ContentItemType, stepSlug: ConversationStep): string => {
  ...
  const handleStartOver = useCallback(() => {
    ...
    setShowStepPrompt(true);
    if (selectedType) {
      const prompt = getOrbPromptForStep(selectedType, 'intent');
      addTranscriptLine('ai', prompt);
    }
  }, [selectedType, getOrbPromptForStep, addTranscriptLine]);

  // Reveal "Start over" ghost button after 3s of generating so user has an escape hatch
  // without it flashing on fast generations.
  ...
  useEffect(() => {
    if (!isGenerating) { setShowStartOver(false); return; }
    const t = setTimeout(() => setShowStartOver(true), 3000);
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Escape hatch: delayed "Start over" button during script generation */}
            <AnimatePresence>
              {showStartOver && (
                <motion.button
                  key="start-over"
          ...
          {/* Browser compatibility error banner */}
          {browserError && (
            <motion.div
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              data-testid="orb-unsupported-error"
              style={{
            ...
            >
              <Typography variant="small" style={{ color: '#ef4444', margin: 0 }}>
                {browserError}
              </Typography>
            </motion.div>
          )}
