...
import { TestLoginButton } from '@/components/shared/TestLoginButton';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (error) { setError(normalizeAuthError(error)); return; }
        if (data?.session) {
      ...
      <div style={{ width: '100%' }}>
        ...
        <GlassCard variant="auth">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {message && (
              <div style={{ padding: spacing.md, borderRadius: borderRadius.md, background: `${colors.success}20`, border: `1px solid ${colors.success}40`, marginBottom: spacing.lg }}>
                <Typography variant="body" style={{ color: colors.success }}>{message}</Typography>
              </div>
            )}

            {(error || errorParam === 'callback_failed') && (
              <div style={{ padding: spacing.md, borderRadius: borderRadius.md, background: `${colors.error}20`, border: `1px solid ${colors.error}40`, marginBottom: spacing.lg }}>
                <Typography variant="body" style={{ color: colors.error }}>
                  {error || (errorParam === 'callback_failed' ? t('callbackFailed') : null)}
                </Typography>
              </div>
            )}
            ...
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <Input
                  ...
                  error={errors.password?.message ? tv(errors.password.message as Parameters<typeof tv>[0]) : undefined}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: spacing.xl }}>
              <Link href="/forgot-password" style={{ textDecoration: 'none', color: colors.accent.primary, fontSize: '14px', fontWeight: 500 }}>
                {t('forgotPassword')}
              </Link>
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: spacing.lg }}>
              <Typography variant="body" style={{ color: colors.text.secondary }}>
                {t('noAccount')}{' '}
                <Link href="/signup" style={{ color: colors.accent.primary, textDecoration: 'none', fontWeight: 600 }}>
                  {t('signupLink')}
                </Link>
              </Typography>
            </div>
        ...
        <div style={{ textAlign: 'center', marginTop: spacing.xl }}>
          <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: spacing.sm, color: colors.text.secondary, textDecoration: 'none', fontSize: '14px' }}>
