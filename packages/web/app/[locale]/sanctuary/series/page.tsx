...
export default function SeriesPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('content_series')
      .select(`
        id, title, description, cover_emoji, is_published, is_listed,
        play_count, share_count, created_at,
        content_series_items(count)
      `)
      ...
      .then(({ data }) => {
        setSeries(
          (data ?? []).map((s) => ({
            ...s,
            item_count: (s as { content_series_items?: { count: number }[] }).content_series_items?.[0]?.count ?? 0,
          })),
        );
        setLoading(false);
      });
              ...
              >
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <div style={{ display: 'flex', gap: spacing.lg, flexWrap: 'wrap' }}>
                    <Typography variant="small" style={{ color: colors.text.secondary, fontSize: 12 }}>
                      {s.item_count ?? 0} sessions
                    </Typography>
                    <Typography variant="small" style={{ color: colors.text.secondary, fontSize: 12 }}>
                      <Play size={11} style={{ display: 'inline', marginRight: 3 }} />
                      {s.play_count} plays
                    </Typography>
                    <Typography variant="small" style={{ color: colors.text.secondary, fontSize: 12 }}>
                ...
                <div style={{ display: 'flex', gap: spacing.sm, alignItems: 'center', flexShrink: 0 }}>
                  ...
                  <button
                    ...
                    title={s.is_listed ? 'Unlist from marketplace' : 'List on marketplace'}
                    style={{
                      padding: `${spacing.sm} ${spacing.md}`,
                      borderRadius: borderRadius.md,
                      background: s.is_listed ? `${colors.accent.primary}15` : colors.glass.light,
                      border: `1px solid ${s.is_listed ? colors.accent.primary + '40' : colors.glass.border}`,
                      color: s.is_listed ? colors.accent.primary : colors.text.secondary,
                      cursor: 'pointer',
                      display: 'flex',
                      alignItems: 'center',
                      gap: spacing.xs,
