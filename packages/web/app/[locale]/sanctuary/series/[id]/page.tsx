...
export default function SeriesDetailPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([seriesRes, itemsRes, contentRes]) => {
      if (seriesRes.data) setSeries(seriesRes.data);
      if (itemsRes.data) setItems(itemsRes.data as unknown as SeriesItem[]);
      if (contentRes.data) setOwnedContent(contentRes.data);
      setLoading(false);
    });
  }, [id, user?.id]);

  const handleSaveTitle = async (field: 'title' | 'description' | 'cover_emoji', value: string) => {
    if (!series) return;
  ...
  const handleReorder = async (reordered: SeriesItem[]) => {
    setItems(reordered);
    const updates = reordered.map((item, index) =>
      supabase.from('content_series_items').update({ order_index: index }).eq('id', item.id),
    );
  ...
  return (
    <PageShell intensity="low" allowDocumentScroll>
      <div style={{ maxWidth: CONTENT_MAX_WIDTH, margin: '0 auto', padding: `${spacing.xxl} ${spacing.xl}` }}>
        ...
        <div style={{ marginBottom: spacing.xxl }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: spacing.md, marginBottom: spacing.md, flexWrap: 'wrap' }}>
            <input
              ...
              onBlur={(e) => handleSaveTitle('cover_emoji', e.target.value)}
              style={{ fontSize: 32, background: 'none', border: 'none', outline: 'none', width: 48, cursor: 'text' }}
            />
            <input
              defaultValue={series.title}
              onBlur={(e) => handleSaveTitle('title', e.target.value)}
              style={{
                flex: 1,
                minWidth: 200,
          ...
          <input
            ...
            onBlur={(e) => handleSaveTitle('description', e.target.value)}
            style={{
              width: '100%',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: spacing.xxl, alignItems: 'start' }}>
          {/* Left: session order */}
          <div>
            ...
            {items.length === 0 ? (
              ...
              >
                {items.map((item, idx) => {
                  const tc = TYPE_COLOR[item.content_items?.type] ?? colors.accent.primary;
                  return (
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <Typography variant="small" style={{ color: colors.text.primary, fontWeight: 600, fontSize: 13 }}>
                            {idx + 1}. {item.content_items?.title}
                          </Typography>
                          <Typography variant="small" style={{ color: tc, fontSize: 11, textTransform: 'capitalize' }}>
                            {item.content_items?.type}
                            {item.content_items?.duration ? ` · ${item.content_items.duration}` : ''}
                          </Typography>
                        </div>
                        <button
                          onClick={() => handleRemoveItem(item.id)}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: colors.text.secondary, opacity: 0.5, padding: spacing.xs }}
                        >
                          <Trash2 size={13} />
                        </button>
                      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <Typography variant="small" style={{ color: tc, fontSize: 11, textTransform: 'capitalize' }}>{content.type}</Typography>
                      </div>
                      <button
