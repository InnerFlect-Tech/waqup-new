...
- Active link is derived from `usePathname()` (`next/navigation`): exact match for Library, Create, Profile; "Home" is also active when pathname starts with `/sanctuary` so the nav reflects that Sanctuary is part of the home/sanctuary area.
- Logo: when authenticated, links to `/home` (app dashboard); when public or simplified, links to `/` (landing).
